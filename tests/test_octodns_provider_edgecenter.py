...
class TestEdgeCenterProviderFailover(TestCase):
    ...
    def test_populate(self):
        ...
        # TC: No diffs == no changes
        with requests_mock() as mock:
            base = "https://api.edgecenter.ru/dns/v2/zones/u.test/rrsets"
            with open("tests/fixtures/edgecenter-no-changes-failover.json") as fh:
                mock.get(base, text=fh.read())
    ...
    def test_apply(self):
                ...
                "weight-cname-smpl",
                {
                    ...
                    "dynamic": {
                        "pools": {
                            "weight": {
                                "values": [
                                    {"value": "ru-1.u.test.", "weight": 2},
                                    {"value": "ru-2.u.test.", "weight": 1},
                                    {"value": "eu.u.test.", "weight": 5},
                                ]
                            }
                        },
                        "rules": [{"pool": "weight"}],
        ...
        provider._client._request.assert_has_calls(
            [
                call(
                    "POST",
                    "http://api/zones/u.test/weight-cname-smpl.u.test./CNAME",
                    data={
                        "ttl": 300,
                        "filters": self.weighted_shuffle_filters,
                        "resource_records": [
                            {"content": ["eu.u.test."], "meta": {"weight": 5}},
                            {
                                "content": ["ru-1.u.test."],
                                "meta": {"weight": 2},
                            },
                ...
                call(
                    "POST",
                    "http://api/zones/u.test/weight-simple.u.test./A",
                    data={
                        "ttl": 300,
                        "filters": self.weighted_shuffle_filters,
                        "resource_records": [
